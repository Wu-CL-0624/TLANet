...
def load_and_preprocess_data(train_path, val_path):
    ...
    # 过滤掉标签为'A'的样本
    train_df = train_df[train_df['type'] != 'A']
    val_df = val_df[val_df['type'] != 'A']
    
    # 确保过滤后仍有数据
    if len(train_df) == 0:
        raise ValueError("训练集中没有非A类的样本，请检查数据！")
    if len(val_df) == 0:
        raise ValueError("验证集中没有非A类的样本，请检查数据！")
    ...
    print(f"过滤掉A类后 - 训练集形状: {X_train.shape}, 训练标签形状: {y_train_encoded.shape}")
    print(f"过滤掉A类后 - 验证集形状: {X_val.shape}, 验证标签形状: {y_val_encoded.shape}")
    print(f"剩余类别: {label_encoder.classes_}")
...
class TCNLSTM多头AttentionModel(nn.Module):
    def __init__(self, input_length, num_classes, num_heads=8):
...
def plot_learning_rate(history):
    ...
    plt.tight_layout()
    # 修改为保存SVG格式
    plt.savefig('tcn_lstm_multihead_learning_rate.svg', 
                format='svg', 
                dpi=300, 
                bbox_inches='tight',
                facecolor='white',
...
def evaluate_model(model, val_loader, label_encoder, X_val_raw):
    ...
    plt.tight_layout()
    # 修改为保存SVG格式
    plt.savefig('tcn_lstm_multihead_confusion_matrix.svg', 
                format='svg', 
                dpi=300, 
                bbox_inches='tight',
                facecolor='white',
...
def plot_training_history(history):
    ...
    plt.tight_layout()
    # 修改为保存SVG格式
    plt.savefig('tcn_lstm_multihead_training_history.svg', 
                format='svg', 
                dpi=300, 
                bbox_inches='tight',
                facecolor='white',
...
def main():
    # 数据路径 - 请根据实际情况修改
    train_path = r"/public/home/wuchengliang/ecg_train_val_minmax_withoutA/train.csv"
    val_path = r"/public/home/wuchengliang/ecg_train_val_minmax_withoutA/val.csv"
